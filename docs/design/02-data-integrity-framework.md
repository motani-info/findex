...
| `beta` | REAL | **optional(入手難)** | （要検討） | ROIC-WACC ← 低カバレッジ要因 |
| `cost_of_debt` | REAL | **optional(入手難)** | EDINET(支払利息/有利子負債) | ROIC-WACC ← 低カバレッジ要因 |
| `effective_tax_rate` | REAL | optional | EDINET | ROIC-WACC(NOPAT) |

> **入手難フィールド**（investment_securities / beta / cost_of_debt）が、旧 rules.yaml の低カバレッジ（ROIC48%・FCF/ネットキャッシュ）の主因と推定。D2.5 で実証し、D4 でこれら依存指標の扱い（重み・代替・不足時挙動）を決める。
