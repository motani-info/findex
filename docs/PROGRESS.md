...
| **株数/分割の基準ズレ（多軸一致・最優先）** | 🟩 **T1是正 全件展開Phase2完了（2026-06-24）＝17→7件**。doc11分割補正を開示日基準へ（`disclosed_date` 全件4,654行補完）→`splits --all`＋`derive --all`→triageで1491/7111/8341/9302等が解消。golden18/18・回帰なし。**全件スキャン中に二次バグ発見＝splits空応答洗替で実在分割喪失（1961/1980/2003/UT）→`fetch/splits.py`根治(コミット2c0314b)＋split0件1504再取得で復活（UT 0.76→11.45）**。残7件＝既存残存（2726/5535のk>1株数基準差・8022ミズノ境界等・個別対応）。**Option1=yfinance発行済株数の現在断面採用は検証の上 不採用/revert**（新T1 13件出現・うち12件で報告株数=Yahoo完全一致でyfが誤り＝優先株/金庫株/浮動株でブレる。**報告株数こそ真値**）。mcap ≤5%=84%/中央1.2%。正本=`docs/yahoo-crosscheck-remediation.md` T1節 |
