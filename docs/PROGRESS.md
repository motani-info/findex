...
| **株数/分割の基準ズレ（多軸一致・最優先）** | 🟥 **NEW（2026-06-23 全銘柄多軸突合で確定・17件）**。PBR・PER・時価総額が同係数kで同方向にズレ＋利回りは÷k＝株数/1株指標の基準誤りの確実な符号。例: 1491中外鉱業(k≈19)・7111INEST(k≈15)・6731ピクセラ(k≈7.6)・8341七十七銀行(k≈3.5・利回り÷k一致)・9302三井倉庫(k≈3.6)・8022ミズノ(k≈0.38)。要・権威ソースで株数/分割を裏取り→補正。リスト=`logs/yahoo_ranking_triage_*.csv` |
