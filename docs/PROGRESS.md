...
**最終更新**: 2026-06-16夜 / **現在フェーズ**: **Part2 全データ洗替を実行中**（取得層を背景ジョブで全3734へ流し中）/ **次の一歩**: 取得完走 → `bash scripts/finish_refresh.sh`（治癒→derive --all→score --all→verify --all）。**出力本番化は凍結**（洗替後に解凍）
