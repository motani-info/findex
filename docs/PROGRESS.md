...
| **PBR/自己資本(BPS)の基準・抽出エラー** | 🟩 **T2診断完了（2026-06-24）＝「系統バグでない」と確定・是正余地12件のみ修正**。bps vs equity/shares をユニバース測定→±15%内が90%。>25%乖離153件をYahoo突合: **92件(60%)はfindex正・Yahoo一致**（`Eq`に優先株/少数株主が含まれる良性乖離・一律置換は良データを壊す）／**真の是正余地は黒字×eq/sh=Yahoo一致の12件のみ**→`_BPS_FROM_EQUITY`でbps=equity/sharesへ置換（12/12 Yahoo±15%収束・2978 3.68→2.12等）。残=赤字ボラ/Yahoo誤り(**4222=Yahoo pbr0.39が自己資本>総資産の不可能値＝Yahoo側の誤り**)/鮮度で対応不要。pytest153・golden18/18。正本=`docs/yahoo-crosscheck-remediation.md` T2節 |
