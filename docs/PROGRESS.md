...
**最終更新**: 2026-06-15 / **現在フェーズ**: **設計フェーズ完結（D1〜D7・D5すべて）**/ **次の一歩**: 実装フェーズの方針決め（移行→fetch→derive→score→verify）。D5 X発信は実装が動いてから（Phase6）
...
- **完了（設計フェーズ）**: D1〜D2.7、D3 データモデル、D4 指標システム、D4.5 較正v4、D7 ワークフロー、D6 多フィールド検証、**D5 X発信戦略** ＝ **全設計が完結**
- **次**: **実装フェーズの方針決め**。順序＝移行(legacy→v2)→fetch実装(kabutan/EDINET/J-Quants/yfinance2000)→derive(status付き)→score(v4)→verify→X発信。コホート38社で回す。X発信は実装が動いてから（Phase6）
