...
1. **【柱1・追跡】予想配当の分割データ欠落の追い込み** — `logs/yahoo_highdiv_compare_*.csv` の +乖離クラスタ（分割記録なしで実績の約2倍）を `findex splits` 再取得/補完で潰す。Yahoo突合の最大乖離をさらに縮める。読み取り専用の突合ツールあり（`scripts/yahoo_highdiv_compare.py`）。
