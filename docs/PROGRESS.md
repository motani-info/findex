...
**最終更新**: 2026-06-15 / **現在フェーズ**: **実装フェーズ進行中（導出層Phase3-a〜3-d完了）**/ **次の一歩**: beta算出（株価×TOPIX回帰）→ROIC−WACC活性化、続いてPhase 3-e（5状態status確定＋claim別グレード組立）
...
| 3 | 導出（status付き・YoC質ゲート・DOE・打ち切り合成・**seam穴橋渡し**） | golden_streaks緑・status分布妥当 | 🟨 3-a〜3-d完了・beta/3-e残 |
...
- **3-d 価格由来指標（6指標）**: PER=価格/EPS・PBR=価格/BPS・時価総額=価格×株数・配当利回り=DPS/価格・ミックス係数=PER×PBR・ネットキャッシュPER。最新終値×最新J-Quants確報の1株/株数。コホート34/35ok（日産=赤字でPER系insufficient・クレディセゾン=J-Quants基礎データ無でPBR/時価missing・メルカリ=無配で利回missing＝全て正当）。実値も妥当（花王PER23.4/PBR2.59、JT時価12兆、メルカリPBR6.2成長株プレミアム）。**beta（株価×TOPIX回帰）はTOPIX未取得で別タスク→ROIC−WACC活性化の前提**
