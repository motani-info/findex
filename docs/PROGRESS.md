...
| **予想配当の元になる分割データ欠落** | 🟨 **NEW（2026-06-22突合で発覚）**。+乖離で分割記録なし（2296伊藤ハム/5410合同製鐵/9629PCA等・forecast==実績なのに約2倍）＝yfinanceの分割データ欠落の疑い（柱1=分割カバレッジの別軸）。候補リスト=`logs/yahoo_highdiv_compare_*.csv` |
| **BPS/株数の基準・抽出エラー（個別）** | 🟨 **NEW（2026-06-23 Yahooランキング多軸突合で発覚）**。価格は一致するのにPBRが大きく外れる個別銘柄＝BPS/株数の基準ズレ。確証例=**4222児玉化学**（findex BPS260/株数1567万 vs Yahoo BPS1829/株数約300万・7倍乖離・分割記録なし・FY24→25で株数倍増しequity横ばい）。`scripts/yahoo_ranking_snapshot.py` で継続検出 |
| **時価総額の株数基準差（自己株）** | 🟨 **NEW（2026-06-23）**。時価総額は中央3.4%で良好だが、+10〜27%乖離する大型株群（5803フジクラ/6702富士通/7267ホンダ/7201日産/7259アイシン…）＝発行済株式数 vs 上場株式数（自己株控除）の基準差の疑い。系統的＝一括検証可能 |
...
1. **【柱1・追跡】Yahoo多軸突合で炙り出した柱1候補の追い込み** — 検証ツール2本（読み取り専用）: 配当=`scripts/yahoo_highdiv_compare.py`／多軸=`scripts/yahoo_ranking_snapshot.py`（配当利回り/PBR実績/PER予想/ROE/時価総額の当日断面→`logs/yahoo_ranking_*.csv`）。**クリーン軸（時価総額 中央3.4%・PBR実績 中央5.3%・配当利回り 中央3.0%）で findex は外部公開値と整合**＝柱1の裏取り。残る具体候補= ①**4222児玉化学のBPS/株数異常**（最優先・確証済）②**時価総額+10〜27%の大型株群＝自己株基準差**（系統的・一括検証）③予想配当の分割欠落クラスタ（`yahoo_highdiv_compare_*.csv`）を `findex splits` 補完。※PER/ROEの大乖離は基準差（予想EPS vs 確報・ROE定義差）＝即バグでない（PERは配当同様「予想化」の改善余地）。
