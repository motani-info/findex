...
- **ゲート**: コホート38社のstocksが会計メタ・listing_dateまで揃う
...
- **ゲート**: コホートで各フィールドのstatus（ok/missing）が想定通り（capex/investment_securitiesがEDINETで埋まる）
...
### Phase 6: X発信（D5・優先度低）
- 実装が安定してから着手。フック（切り口①YoC×持続性等）・品質ゲート連動・Playwright投稿
