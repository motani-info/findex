...
社名入りに改稿した ROIC−WACC 例を提示し全テーマ横展開を依頼。4パターンを実データで提示し、
ユーザーは **P2「社名＋看板指標」** を選択。正本: [16-post-body-top3-names.md](design/16-post-body-top3-names.md)。

- 本文を `フック→テーマ名トップN→トップ3社ブロック→フック→#タグ` 構成へ。社名横の数字は
  **そのテーマのランキングを決めた指標（sort_key 由来）**＝看板と一致し最も誠実（headline=(key,fmt)）。
- 社名は **NFKC 半角正規化**（ＺＯＺＯ→ZOZO）＋12字省略。`_post_name`/`_body_metric`/`_post_name_block` 新設。
- **本文上限ゲートを 140→250 に引き上げ**（旧140はXの実上限280の半分で過保守。社名3行で超過）。`BODY_MAX`。
- 12宣言テーマ＋カスタム5本に注入。large_cap のみ並びは総合スコア順だが看板「配当」の引きで利回りを添える。

検収: **pytest 118 passed**（新規5）/ golden **18/18 不整合0** / post-gallery --all 17テーマ再生成
（全本文トップ3社＋看板指標・最大188/250字・全ゲート通過）。
