...
# ══ 売られすぎ（直近の株価下落）指標 ════════════════════════════════════
# 「売られすぎ高配当」テーマの核＝直近で株価が下げた度合い。price_history（既取得・2000〜）
# のみ由来で**新規フェッチ不要**＝レート制限の鉄則に無関係。最新終値を基準に point-in-time で
# 算出する（後日走らせれば自然に別の結果＝ユーザー要件「その時その時の情報」）。
#   drawdown_from_high … 直近52週高値からの下落率。0〜1（大きいほど売られすぎ）。
#   price_return_1y/6m  … 1年/6ヶ月騰落率（負＝下落）。
# 確証主義: 履歴が窓を十分カバーしない（新規上場直後など）は数字を出さず insufficient。
_DRAWDOWN_COLS = ["price_high_52w", "drawdown_from_high", "price_return_1y", "price_return_6m"]

# 騰落率の基準価格を探す窓（target を中心に前後へこの日数だけ許容＝休場/データ欠落を吸収）。
_RETURN_LOOKUP_SLACK_DAYS = 45
# 52週ドローダウンを「52週高値」と正直に呼べる最小の実履歴スパン（日）。これ未満は insufficient。
_DRAWDOWN_MIN_SPAN_DAYS = 300


def _return_over(conn, code: str, latest_iso: str, current: float, days: int) -> float | None:
    """latest から days 日前の終値に対する騰落率。基準日近傍に終値が無ければ None（insufficient）。"""
    target = (date.fromisoformat(latest_iso) - timedelta(days=days)).isoformat()
    floor = (date.fromisoformat(target) - timedelta(days=_RETURN_LOOKUP_SLACK_DAYS)).isoformat()
    r = conn.execute(
        "SELECT close_adj FROM price_history WHERE code=? AND close_adj>0 AND date<=? AND date>=? "
        "ORDER BY date DESC LIMIT 1",
        (code, target, floor),
    ).fetchone()
    if not r or not r[0]:
        return None
    return round((current - r[0]) / r[0], 6)


def compute_drawdown_metrics_for_code(conn, code: str) -> dict | None:
    """売られすぎ指標（price_history のみ由来・point-in-time）。最新終値が無ければ None。"""
    pr = _latest_price(conn, code)
    if not pr:
        return None
    latest_iso, current = pr
    out: dict = {c: None for c in _DRAWDOWN_COLS}
    status: dict[str, str] = {}

    window_floor = (date.fromisoformat(latest_iso) - timedelta(days=365)).isoformat()
    win = conn.execute(
        "SELECT MAX(close_adj), MIN(date) FROM price_history "
        "WHERE code=? AND close_adj>0 AND date>=? AND date<=?",
        (code, window_floor, latest_iso),
    ).fetchone()
    high_52w, oldest = (win or (None, None))
    # 52週高値＆ドローダウン: 窓内の実履歴スパンが十分（≒1年）なときだけ「52週高値」と称する。
    if high_52w and high_52w > 0 and oldest is not None and \
            (date.fromisoformat(latest_iso) - date.fromisoformat(oldest)).days >= _DRAWDOWN_MIN_SPAN_DAYS:
        out["price_high_52w"], status["price_high_52w"] = round(high_52w, 4), "ok"
        dd = max(0.0, (high_52w - current) / high_52w)   # 高値更新中なら0（売られすぎでない）
        out["drawdown_from_high"], status["drawdown_from_high"] = round(dd, 6), "ok"
    else:
        status["price_high_52w"] = "insufficient"
        status["drawdown_from_high"] = "insufficient"

    for col, days in (("price_return_1y", 365), ("price_return_6m", 182)):
        v = _return_over(conn, code, latest_iso, current, days)
        if v is None:
            status[col] = "insufficient"
        else:
            out[col], status[col] = v, "ok"

    out["_status"] = status
    out["_price_date"] = latest_iso
    return out


def build_drawdown_metrics(conn, codes: list[str]) -> dict:
    """売られすぎ指標を computed_metrics へ書き込む（price 由来・status をマージ）。"""
    now = datetime.now().isoformat(timespec="seconds")
    n = 0
    ok_counts: dict[str, int] = {c: 0 for c in _DRAWDOWN_COLS}
    set_clause = ",".join(f"{c}=excluded.{c}" for c in _DRAWDOWN_COLS)
    insert_sql = (
        f"INSERT INTO computed_metrics (code,{','.join(_DRAWDOWN_COLS)},status_json,price_computed_at) "
        f"VALUES (?,{','.join('?' * len(_DRAWDOWN_COLS))},?,?) "
        f"ON CONFLICT(code) DO UPDATE SET {set_clause},"
        f"status_json=excluded.status_json,price_computed_at=excluded.price_computed_at"
    )
    for code in codes:
        d = compute_drawdown_metrics_for_code(conn, code)
        if not d:
            continue
        status_json = _merge_json(conn, code, "status_json", d.pop("_status"))
        conn.execute(insert_sql, (code, *[d[c] for c in _DRAWDOWN_COLS], status_json, now))
        for c in _DRAWDOWN_COLS:
            if d[c] is not None:
                ok_counts[c] += 1
        n += 1
    conn.commit()
